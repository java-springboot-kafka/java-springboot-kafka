...
What sealed classes are, exactly how they work and why we need them, I will explain in a separate article due to the scope of the topic: [Sealed Classes in Java](https://www.happycoders.eu/de/java/sealed-classes-de/)

_(Sealed classes were first introduced in [Java 15](https://www.happycoders.eu/de/java/java-15-features-de/#Sealed_Classes_Preview) as a preview feature. Three small changes were released in [Java 16.](https://www.happycoders.eu/de/java/java-16-features-de/#Sealed_Classes_Second_Preview) [JDK Enhancement Proposal 409](https://openjdk.org/jeps/409) declares sealed classes production-ready in Java 17 with no further changes.)_
...
In [Java 16](https://www.happycoders.eu/de/java/java-16-features-de/#Strongly_Encapsulate_JDK_Internals_by_Default) , the default mode was changed from "Relaxed Strong Encapsulation" to "Strong Encapsulation". Since then, access to pre-Java 9 packages has also had to be explicitly allowed.
...
Until now, it has been tedious to replace the [random number-generating](https://www.happycoders.eu/de/java/zufallszahl/) classes `Random`and `SplittableRandom`in an application (or even to replace them with other algorithms) even though they offer a largely identical set of methods (e.g. `nextInt()`, `nextDouble()`and stream-generating methods such as `ints()`and `longs()`).

So far, the class hierarchy looked like this:

![Pre-Java 17 Pseudo-Random Number Generators](https://www.happycoders.eu/wp-content/uploads/2021/12/random_number_generators_pre_java17-600x249.png)
...
![Java 17 Pseudo-Random Number Generators](https://www.happycoders.eu/wp-content/uploads/2021/12/random_number_generators_java17-800x438.png)
...
In Java 16 ["Pattern Matching for instanceof"](https://www.happycoders.eu/de/java/java-16-features-de/#Pattern_Matching_for_instanceof) was introduced. This made explicit casts after `instanceof`tests superfluous. This allows e.g. B. Code like the following:
...
Pattern matching can also be used for [`switch`expressions](https://www.happycoders.eu/de/java/switch-expressions/#switch-als-ausdruck-mit-ruckgabewert) ( i.e. `switch`with a return value):
...
Incidentally, when using [sealed classes](https://www.happycoders.eu/de/java/sealed-classes-de/) , the compiler can check whether `switch`the statement or expression is complete. If that is the case, a `default`case is not needed.
...
"Pattern Matching for switch" will be presented again in [Java 18](https://www.happycoders.eu/de/java/java-18-features-de/#Pattern_Matching_for_switch_Second_Preview) as a preview feature and will probably be ready for production in Java 19.
...
1.  The Foreign Memory Access API (introduced in [Java 14](https://www.happycoders.eu/de/java/java-14-features-de/#Foreign-Memory_Access_API_Incubator) , refined in [Java 15](https://www.happycoders.eu/de/java/java-15-features-de/#Foreign-Memory_Access_API_Second_Incubator) and [Java 16](https://www.happycoders.eu/de/java/java-16-features-de/#Foreign_Linker_API_Incubator_Foreign-Memory_Access_API_Third_Incubator) ),
2.  The Foreign Linker API (introduced in [Java 16](https://www.happycoders.eu/de/java/java-16-features-de/#Foreign_Linker_API_Incubator_Foreign-Memory_Access_API_Third_Incubator) ).
...
This is still in the incubator stage, so it may still be subject to significant changes. [I will introduce the new API in the Java 19](https://www.happycoders.eu/de/java/java-19-features/#Foreign_Function_Memory_API_Preview) article when it reaches the preview stage.

### Vector API (Second Incubator)

[As already described in the article about Java 16](https://www.happycoders.eu/de/java/java-16-features-de/#Vector_API_Incubator) , the Vector API is not about the old `java.util.Vector`class, but about mapping mathematical vector calculations to modern CPU architectures with single-instruction-multiple-data (SIMD) support.
...
For this reason, RMI Activation was marked as "deprecated" in [Java 15](https://www.happycoders.eu/de/java/java-15-features-de/#Deprecate_RMI_Activation_for_Removal) and completely removed in Java 17 by [JDK Enhancement Proposal 407 .](https://openjdk.org/jeps/407)

### Remove the Experimental AOT and JIT Compiler

In Java 9, Graal was added to the JDK as an experimental ahead-of-time (AOT) compiler. In [Java 10](https://www.happycoders.eu/de/java/java-10-features-de/#Experimental_Java-Based_JIT_Compiler) , Graal was also made available as a just-in-time (JIT) compiler.
...
The code is an extension of the AArch64 ports for Linux and Windows released in Java 9 and [Java 16 with macOS-specific adjustments.](https://www.happycoders.eu/de/java/java-16-features-de/#WindowsAArch64_Port)
...
!["NEW" page in JavaDoc generated since Java 17](https://www.happycoders.eu/wp-content/uploads/2021/12/javadoc_since_java_17-800x503.png)
...
![Java 16 DEPRECATED page](https://www.happycoders.eu/wp-content/uploads/2021/12/javadoc_deprecated_java_16-800x435.png)

Java 16 DEPRECATED page

Starting with Java 17, we see the features marked as "deprecated" grouped by release:

![Java 17 DEPRECATED page](https://www.happycoders.eu/wp-content/uploads/2021/12/javadoc_deprecated_java_17-800x460.png)
...
[Java 18](https://www.happycoders.eu/de/java/java-18-features-de/) is just around the corner and the features it contains are certain. I will present these in the next article. Would you like to be informed when the article goes online? Then [click here](https://www.happycoders.eu/de/java/java-17-features-de/#) to sign up for the HappyCoders newsletter.